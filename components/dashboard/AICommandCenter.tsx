...
import { Sparkles, Mic, MicOff, ArrowUp, Plus, FileText, MessageCircle, ChevronDown, ArrowDown } from 'lucide-react'
...
export function AICommandCenter({ initialChannel, initialProduct }: AICommandCenterProps) {
    ...
    const generateScript = async (payload: any, refinement?: string) => {
        ...
        try {
            ...
            // Scroll to result with snap behavior
            setTimeout(() => {
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <div className="min-h-[80vh] flex flex-col items-center justify-center w-full relative px-4 py-8">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center animate-in fade-in duration-500">
                        <div className="w-20 h-20 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center mb-6 animate-pulse">
                            <Sparkles className="w-10 h-10 text-white" />
                        </div>
                        <TextShimmerWave className='text-2xl md:text-3xl font-bold mb-3'>
                    ...
                    <>
                        ...
                        {!initialProduct && !result && (
                            <div className={cn(
                                "transition-all duration-500 ease-in-out flex flex-col items-center mb-8",
                        ...
                        {/* Main Card - Only show when not loading and no result */}
                        {!result && (
                            <div
                                ref={containerRef}
                                className={cn(
                                    "w-full max-w-3xl mx-auto bg-gray-100 border border-gray-200 rounded-[2rem] shadow-sm overflow-hidden transition-all duration-300 ease-out",
                                    isExpanded && activeMode === 'script' ? "max-w-4xl" : ""
                                )}
                            >
                                <div className="p-4">
            ...
            {/* Results Section */}
            <div ref={resultRef} className="w-full min-h-screen bg-gradient-to-b from-gray-50 to-white pt-12 px-4 pb-20">
                <div className="max-w-5xl mx-auto">

                    {error && (
                        <div className="p-6 bg-red-50 text-red-600 rounded-2xl text-center my-8 max-w-lg mx-auto">
                            <p className="font-medium">{error}</p>
                            <button onClick={handleSubmit} className="mt-3 text-sm underline hover:no-underline">Tentar novamente</button>
                        </div>
                    )}
